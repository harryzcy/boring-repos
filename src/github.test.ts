...
import { createRepositoryLabel, getAppUserID, getRepositories, getRepositoryLabels } from './github.js'
...
test('Manage repository labels', async () => {
  ...
  const resp = await octokit.request('GET /repos/{owner}/{repo}/labels/{name}', {
    owner,
    repo,
    name: newLabel
  })
